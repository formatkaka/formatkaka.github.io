...
const STARTER_PERSONAS = PRESET_PERSONAS.slice(0, 8);
const MORE_PERSONAS = PRESET_PERSONAS.slice(8);

function PersonaOptions() {
  return (
    <>
      <optgroup label="Starter personas">
        {STARTER_PERSONAS.map((preset) => (
          <option key={preset.id} value={preset.id}>
            {preset.label}
          </option>
        ))}
      </optgroup>
      <optgroup label="More personas">
        {MORE_PERSONAS.map((preset) => (
          <option key={preset.id} value={preset.id}>
            {preset.label}
          </option>
        ))}
...
export function LLMCard(props: LLMCardProps) {
  ...
  const [selectedPreset, setSelectedPreset] = useState(currentPreset?.id || CUSTOM_PERSONA_ID);
  const [customPersona, setCustomPersona] = useState(currentPreset ? '' : persona);
  const selectedLabel =
    PRESET_PERSONAS.find((p) => p.id === selectedPreset)?.label || 'Custom persona';
  ...
  return (
    <>
      <div className="relative flex items-center gap-4 rounded-2xl bg-white px-4 py-4 shadow-sm dark:bg-[#191f2b] dark:shadow-black/20 sm:hidden">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex min-w-0 items-center gap-2">
            <div className="truncate text-lg font-semibold text-[#172238] dark:text-[#f0f2f6]">
              {selectedLabel}
            </div>
            <ChevronDown
              aria-hidden="true"
              className="h-5 w-5 shrink-0 text-[#9196a0] dark:text-[#929aaa]"
              strokeWidth={2.25}
            />
          </div>
      ...
      <div className="hidden sm:block sm:min-h-56 sm:rounded-3xl sm:bg-white sm:px-7 sm:py-7 sm:text-center sm:dark:bg-[#191f2b]">
        {/* Avatar */}
        <div
          className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-base font-bold text-white sm:mx-auto sm:mb-5 sm:h-16 sm:w-16"
          style={{ backgroundColor: color }}
        >
          {provider.charAt(0).toUpperCase()}
        </div>
        ...
        {/* Name */}
        <div className="min-w-0 text-base font-semibold text-[#1b2021] dark:text-[#f0f2f6] sm:mb-3 sm:text-sm">
          {label}
        </div>
        <select
          aria-label={`${label} persona`}
